...
void opcontrol() {
    while (true) {
        if (master.get_digital(pros::E_CONTROLLER_DIGITAL_L1)) {
            // Toggle the motors' state
            if (!motorsActivated) {
                // If motors are not activated, turn them on to 25% speed
                armGroup.move_relative(90, 100);
                motorsActivated = true; // Mark the motors as activated
            } else {
                // If motors are activated, stop them
                armGroup.move_relative(-90, 100);; // Stop
                motorsActivated = false; // Mark the motors as deactivated
        }
...
void controlPiston() {
    while (true) {
        ...
        pros::delay(20); // Small delay for button press processing
    }
}
